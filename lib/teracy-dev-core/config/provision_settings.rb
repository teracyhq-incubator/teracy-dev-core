...
module TeracyDevCore
  module Config
    # legacy, could be refactored soon
    class ProvisionSettings
      @@logger = TeracyDev::Logging.logger_for('Config::ProvisionSettings')
      def self.configure(type, provision, config)
        case type
        when "file"
        ...
        when "salt"
          ...
          logger.warn("no matching config type for: #{type}")
        end
      end
      ...
      def self.file_settings(file, config)
        file.source = config['source']
        file.destination = config['destination']
      end

      # https://www.vagrantup.com/docs/provisioning/shell.html
      def self.shell_settings(shell, config)
      ...
      def self.cfengine_settings(cf, config)
      ...
      def self.chef_common(chef, config)
      ...
      def self.chef_solo_settings(chef, config)
      ...
      def self.chef_zero_settings(chef, config)
        chef_common(chef, config)
        unless config['cookbooks_path'].nil?
          chef.cookbooks_path = config['cookbooks_path']
        end
      ...
      def self.chef_client_settings(chef, config)
        chef_common(chef, config)
      ...
      def self.chef_apply_settings(chef, config)
        chef_common(chef, config)
      ...
      def self.docker_settings(docker, config)
      ...
      def self.puppet_apply(puppet, config)
        unless config['binary_path'].nil?
      ...
      def self.puppet_server(puppet, config)
        unless config['binary_path'].nil?
          puppet.binary_path = config['binary_path']
        end
      ...
      def self.salt_settings(salt, config)
